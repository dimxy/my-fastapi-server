...
def test_update_password_me(
    ...
    user_query = select(UserDB).where(UserDB.email == settings.FIRST_SUPERUSER)
    user_db = db.exec(user_query).first()
    assert user_db
...
def test_register_user(client: TestClient, db: Session) -> None:
    ...
    user_query = select(UserDB).where(UserDB.email == username)
    user_db = db.exec(user_query).first()
    assert user_db
...
def test_delete_user_me(client: TestClient, db: Session) -> None:
    ...
    result = db.exec(select(UserDB).where(UserDB.id == user_id)).first()
